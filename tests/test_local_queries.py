...
class TestGetAvailableDates:
    ...
    @patch('iwork.local_queries.HistoricalSyncState.objects')
    def test_returns_date_objects(self, mock_objects):
        """返回 date 对象列表"""
        from iwork.local_queries import get_available_dates

        mock_objects.using.return_value.filter.return_value.values_list.return_value.order_by.return_value = [
    ...
    @patch('iwork.local_queries.HistoricalSyncState.objects')
    def test_only_returns_successful_snapshots(self, mock_objects):
        """只读取发布成功的历史快照日期。"""
        from iwork.local_queries import get_available_dates

        get_available_dates('local')
        mock_objects.using.return_value.filter.assert_called_once_with(status='success')
...
class TestGetAllStepnos:
    ...
    def test_filters_zero_qty(self, mock_records):
        """qty=0 的工序不返回"""
...
class TestGetBatchBasicStats:
    ...
    @patch('iwork.local_queries.get_records_queryset')
    def test_includes_workorder_count(self, mock_records):
...
class TestGetBatchWorkordersList:
    ...
    @patch('iwork.local_queries.get_records_queryset')
    def test_returns_per_stepno_workorders(self, mock_records):
        """返回按工序分组的工单"""
...
class TestSignatureEquivalence:
    ...
    def test_both_modules_have_batch_functions(self):
        ...
        for remote_func, local_func in [(get_batch_basic_stats, l_basic), (get_batch_hourly_stats, l_hourly),
    ...
    def test_signature_new_flow_functions(self):
        ...
        for remote_func, local_func in [(get_all_flows, l_af), (get_batch_flow_overview, l_fo),
...
class TestGetAllFlowsLocal:
    ...
    @patch('iwork.local_queries.get_records_queryset')
    def test_returns_sorted_flows_excluding_zero_qty(self, mock_records):
...
class TestGetBatchFlowHourlyLocal:
    ...
    @patch('iwork.local_queries.get_records_queryset')
    def test_returns_hourly_data_per_flow(self, mock_records):
        """返回每个 Flow 的每小时产量数据"""
        ...
        mock_qs.exclude.return_value = mock_qs
        mock_qs.filter.return_value = mock_qs
        mock_qs.annotate.return_value.values.return_value.annotate.return_value.order_by.return_value = [
...
class TestGetBatchFlowEmployeesLocal:
    ...
    def test_returns_employees_per_flow_sorted_by_total_qty_desc(
        self, mock_records, mock_metadata,
    ):
...
class TestGetBatchStepnoEmployeesLocal:
    ...
    @patch('iwork.local_queries.get_records_queryset')
    def test_returns_employees_per_stepno_sorted_by_qty_desc(self, mock_records):
