...
## 14.4 2026-08-31 P1缺口修复切片（本地已验证，CI失败，生产未执行）

本轮针对最终修改方案中列出的六项P1缺口继续实施，进度仍以本总方案为唯一基线。
代码已提交并推送为`2590c52f2d273816eab91f08d4dab96d7cc0b5ee`，随后触发真实CI
[`33370363488`](https://github.com/GuChenkano/iwork/actions/runs/33370363488)。该CI失败后未触发Release、
更新生产准入或操作生产主机；旧Commit、旧Release、旧预检和旧确认均不可复用。
...
- 固定部署脚本当前字节哈希为
  `c5a058a1e3243ab7fd2c68c00448c0d1976b060e5a035a4195171a89189185fb`，已同步到本轮Workflow门禁。
...
详见[Owner/ACL专项规范](2026-08-31-iwork-Owner-ACL校验规范.md)，修复前不得重试Release或生产动作。

本次Hosted Runner阻断修复已完成代码切片但尚未重新触发真实CI：部署脚本新增固定
`iwork-owner-acl/v1` trust manifest（路径与字节SHA-256必须同时提供），以SID化Owner、关闭继承、显式读写白名单、
重解析点和完整变更权限掩码校验探针生产者、签名材料与收据目录；管理员组、Users、Everyone和Authenticated Users
不得进入清单授权集合。隔离夹具在临时目录生成当前Runner SID+SYSTEM的受保护ACL，继续调用同一严格校验器，
不再要求或自动解析`DONGMING\\shuju`。Hosted Runner的临时Owner兼容性因此由测试清单显式证明，生产路径仍要求真实主机清单、
生产者与签名材料，未提供这些材料前`apply=true`继续失败关闭。
...
- 本仓库绑定代码变更：已完成并提交为`9ab0bf6`，已推送`jiejiejie113/iwork11`的
  `Keycloak`与`local-sync-b7b7b76`分支（原`Keycloak`为`c4cca04`，本次为快进推送）。
...
- 2026-09-21本机端到端实测：`dist/PCI-iwork-offline-20260921-112742.zip`（270文件，
  ...
  MANIFEST路径统一正斜杠，270项全部与压缩包条目哈希一致（独立校验无缺失/无多余）。
