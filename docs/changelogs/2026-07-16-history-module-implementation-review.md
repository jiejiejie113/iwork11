...
构建成功后重试原 GET，并显示已加载的历史日期和源记录数。快照版本仅保留在 API 和
运维状态中，不在业务界面展示。已有成功快照时确保接口直接
...
快照时页面显示“正在准备历史数据”，完成后显示“已加载历史数据”和源记录数。

### 8.2 生产详情日期状态

生产详情从 URL 查询参数读取日期。没有日期时使用 `Asia/Bangkok` 的业务日期。

历史日期显示绿色“已加载历史数据”状态，日期切换后重新加载当前视图。
...
和本地原子发布后，前端重试原 GET。历史看板显示已加载日期和源记录数；生产详情显示
“已加载历史数据”。快照版本继续供 API 和运维对账，但不显示给业务用户。构建失败时
清空旧日期统计并显示错误，不把旧数据或空列表当成
...
- 用户界面不再显示 `v1`、`v2` 等快照版本号；版本字段继续保留在后端状态和 API 中，
  只用于运维排查和对账。
- 原“本地历史快照”文案统一改为“已加载历史数据”。
- 历史看板日期上限为曼谷业务日期的前一天，因为当天生产尚未结束，不能生成历史快照。
- 生产详情日期上限为曼谷业务日期当天，今日仍可查看实时数据，未来日期不可选。
- 手工在生产详情 URL 中填写未来日期时，前端会将日期钳制到曼谷业务日期当天。
