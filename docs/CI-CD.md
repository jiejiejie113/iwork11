...
- GitHub托管Runner运行Python 3.11测试、Ruff、迁移同步检查、PowerShell脚本测试并校验Compose配置。
- pytest固定加载`iwork.test_settings`，三套数据库均为内存SQLite，缓存和Celery也使用进程内实现。
- Compose配置校验使用仅供解析的占位变量，不加载`env/production.env`或`D:\DM\dkt-secrets.env`。
- 测试Job只删除可能被自动加载的仓库根`.env`，保留经过无密码契约检查的`env/local.env`和`env/production.env`，并在测试Job内完成Compose配置解析。
- 不执行`docker compose up`、`deploy.ps1`、SSH、数据库迁移或服务器健康探测。
- 完整CI不构建、不发布GHCR镜像；生产候选镜像只在Release阶段构建一次并原样发布。

## 触发方式

- 完整 `iwork CI`（`.github/workflows/ci.yml`）只接受 GitHub Actions 页面手工触发，并要求提供唯一的`request_id`。
- `.github/workflows/ci-pr.yml`独立执行面向`Keycloak`的轻量 Pull Request 静态检查，不触发完整 CI。

同一 Commit 的重复手工运行不会自动取消，避免完整 CI 被其他请求静默中断。
...
Docker Compose配置解析需要三个仅供解析的占位变量，禁止填写生产值：
